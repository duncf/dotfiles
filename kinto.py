...
# (overrides some bindings from general file manager code block below)
define_keymap(
    re.compile("^dde-file-manager$", re.IGNORECASE),
    {
...
# (overrides some bindings from general file manager code block below)
define_keymap(
    re.compile("^dolphin$", re.IGNORECASE),
    {
...
# (overrides some bindings from general file manager code block below)
define_keymap(
    re.compile("^io.elementary.files$", re.IGNORECASE),
    {
...
define_keymap(
    re.compile("^org.gnome.nautilus$|^nautilus$", re.IGNORECASE),
    {
        ...
        K("RC-comma"): K(
            "RC-comma"
        ),  # Overrides "Open preferences dialog" shortcut below
    },
...
# (overrides some bindings from general file manager code block below)
define_keymap(
    re.compile("^pcmanfm$|^pcmanfm-qt$", re.IGNORECASE),
    {
        K("RC-KEY_2"): K("C-KEY_4"),  # View as List (Detailed) [Not in PCManFM-Qt]
        K("RC-Backspace"): [
            K("Delete"),
            K("Enter"),
        ],  # Move to Trash (delete, bypass dialog)
    },
...
# (overrides some bindings from general file manager code block below)
define_keymap(
    re.compile("^spacefm$", re.IGNORECASE),
    {
...
# (overrides some bindings from general file manager code block below)
define_keymap(
    re.compile("^thunar$", re.IGNORECASE),
    {
...
define_keymap(
    re.compile("^Firefox$", re.IGNORECASE),
    {
        ...
        K("RC-Shift-N"): K(
            "RC-Shift-P"
        ),  # Open private window with Ctrl+Shift+N like other browsers
    },
)
...
define_keymap(
    re.compile("^ulauncher$", re.IGNORECASE),
    {
        K("RC-Key_1"): K(
            "Alt-Key_1"
...
define_keymap(
    re.compile(
        "^org.gnome.Console$|^Kgx$|^deepin-terminal$|^Angry*IP*Scanner$|^jDownloader$",
        re.IGNORECASE,
    ),
    {
        ### Tab navigation
...
define_keymap(
    re.compile(sublimeStr, re.IGNORECASE),
    {
        # K("Super-c"): K("LC-c"),                    # Default - Terminal - Sigint
        # K("Super-x"): K("LC-x"),                    # Default - Terminal - Exit nano
        # K("Alt-c"): K("LC-c"),                        #  Chromebook/IBM - Terminal - Sigint
...
define_keymap(
    re.compile("^deepin-terminal$", re.IGNORECASE),
    {
...
define_keymap(
    re.compile("alacritty", re.IGNORECASE),
    {
